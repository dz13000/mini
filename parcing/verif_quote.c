...
/*   Updated: 2023/10/26 13:25:11 by cabouzir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (line[++i])
	{
		if (line[i] == '\'' || line[i] == '\"')
		{
			if (quote == 0 && line[i] == '\'')
				...
				quote = 1;
			while (line[++i] && ((line[i] != 34 && quote == 1) || (line[i] != 39
						&& quote == 2)))
			{
				line[i] = line[i] * -1;
			}
...
void	ft_positive(t_data *data)
{
	...
	{
		i = 0;
		if (tmp->next != NULL && (tmp->token == SIMPLE_QUOTE
...
void	replace_dollar_quote(t_list *lst)
{
	...
	while (lst->content[i])
	{
		...
		{
			if (ft_isalpha(lst->content[i + 1]) == 1 || lst->content[i
					+ 1] == '_')
